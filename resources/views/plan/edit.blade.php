    ...
    <body class="bg-gray-50 flex h-screen overflow-hidden">
        ...
        <div id="main-content" class="flex-1 flex flex-col overflow-y-auto overflow-x-hidden">
            ...
            <main class="p-4 md:p-8 flex-1">
                <h2 class="text-lg font-bold mb-4 text-red-600">Edit Plan</h2>
                <form class="space-y-4 md:space-y-6 mt-3" method="POST" action="{{ route('update-plan', $data->id) }}">
                    @csrf
                    @method('PUT')
                    <div>
                        <label for="title"
                    ...
                    <div>
                        ...
                        <input type="text" name="price" id="price"
                            ...
                            value="{{ $data->price }}" placeholder="Masukan price ...." />
                        @error('price')
                            <p class="text-sm text-red-600 mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    <div>
                        ...
                        <input type="text" name="duration" id="duration"
                            ...
                            value="{{ $data->duration }}" placeholder="Masukan durasi per hari ...." />
                        @error('duration')
                            <p class="text-sm text-red-600 mt-1">{{ $message }}</p>
                        @enderror
                    </div>
