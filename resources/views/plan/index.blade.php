...
@section('content')
    ...
    <body class="bg-gray-50 flex h-screen overflow-hidden">
                        ...
                        <tbody>
                            @foreach ($data as $news)
                                <tr class="bg-white border-b border-gray-200">
                                    ...
                                    <td class="px-6 py-4">
                                        {{ number_format($news->price, 0, ',', '.') }}
                                    </td>
                                    <td class="px-6 py-4">
                                        {{ $news->duration }} Hari
                                    </td>
                                    <td class="px-6 py-4">
                                        {{ \Illuminate\Support\Str::limit(strip_tags($news->description), 50) }}
                                    </td>
                                    <td class="px-6 py-4">
                                        <div class="flex justify-content-center">
                                            <a href="/plan/{{ $news->id }}" class="text-[18px] mx-2">
        ...
        <x-modal>
            <h2 class="text-lg font-bold mb-4 text-red-600">Tambah Plan</h2>
            <form class="space-y-4 md:space-y-6 mt-3" method="POST" action="{{ route('store-plan') }}">
                @csrf
                <div>
                ...
                <div>
                    <label for="price"
                        class="block mb-2 text-sm font-medium text-black-100 dark:text-black">Price</label>
                ...
                <div>
                    <label for="duration"
                        class="block mb-2 text-sm font-medium text-black-100 dark:text-black">Duration</label>
                    <input type="text" name="duration" id="duration"
                        ...
                        value="{{ old('duration') }}" placeholder="Masukan durasi per hari ...." />
                    @error('duration')
                        <p class="text-sm text-red-600 mt-1">{{ $message }}</p>
                    @enderror
                </div>
                <div>
                    <label for="description"
                        class="block mb-2 text-sm font-medium text-black-100 dark:text-black">Description</label>
        ...
        <script>
            ...
            $(document).on('click', '.delete-course', function() {
                ...
                Swal.fire({
                    title: 'Yakin hapus Plan?',
                    text: 'Menghapus plan akan menghapus semua data langganan yang terkait secara permanen.Anda tidak dapat membatalkan tindakan ini.',
                    icon: 'warning',
