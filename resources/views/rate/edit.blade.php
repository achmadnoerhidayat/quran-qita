    ...
    <body class="bg-gray-50 flex h-screen overflow-hidden">
        ...
        <div id="main-content" class="flex-1 flex flex-col overflow-y-auto overflow-x-hidden">
            ...
            <main class="p-4 md:p-8 flex-1">
                <h2 class="text-lg font-bold mb-4 text-red-600">Edit Nilai Tukar</h2>
                <form class="space-y-4 md:space-y-6 mt-3" method="POST" enctype="multipart/form-data"
                    action="{{ route('update-rate', $data->id) }}">
                    @csrf
                    @method('PUT')
                    <div>
                        <label for="role"
                        ...
                        <select name="type" id="type"
                            ...
                            <option value="purchase" {{ ($data->type ?? '') == 'purchase' ? 'selected' : '' }}>Purchase
                            </option>
                            <option value="withdraw" {{ ($data->type ?? '') == 'withdraw' ? 'selected' : '' }}>Withdraw
                            </option>
                        </select>
                        @error('type')
                    ...
                    <div>
                        ...
                        <input type="text" name="coin_unit" id="coin_unit"
                            ...
                            value="{{ $data->coin_unit }}" placeholder="Masukan Jumlah Koin ...." />
                        @error('coin_unit')
                            <p class="text-sm text-red-600 mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    <div>
                        ...
                        <input type="text" name="unit_value" id="unit_value"
                            ...
                            value="{{ $data->unit_value }}" placeholder="Masukan unit_value ...." />
                        @error('unit_value')
                            <p class="text-sm text-red-600 mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="flex justify-between">
                        <label for="coin_unit"
                            class="block mb-2 text-sm font-medium text-black-100 dark:text-black">Active</label>
                        <div class="relative inline-block w-11 h-5">
                            <input {{ ($data->active ?? '') == 1 ? 'checked checked' : '' }} id="switch-component-1"
                                type="checkbox"
