    ...
    <body class="bg-gray-50 flex h-screen overflow-hidden">
        ...
        <div id="main-content" class="flex-1 flex flex-col overflow-y-auto overflow-x-hidden">
            ...
            <main class="p-4 md:p-8 flex-1">
                <h2 class="text-lg font-bold mb-4 text-red-600">Edit Pengguna</h2>
                <form class="space-y-4 md:space-y-6 mt-3" method="POST" action="{{ route('update-user', $data->id) }}">
                    @csrf
                    @method('PUT')
                    <div>
                        <label for="title"
                            class="block mb-2 text-sm font-medium text-black-100 dark:text-black">Name</label>
                        <input type="text" name="name" id="name"
                            ...
                            value="{{ $data->name }}" placeholder="Masukan Name ...." />
                        @error('name')
                    ...
                    <div>
                        ...
                        <input type="email" name="email" id="email"
                            ...
                            value="{{ $data->email }}" placeholder="Masukan email ...." />
                        @error('email')
                            <p class="text-sm text-red-600 mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    <div>
                        <label for="role"
                            class="block mb-2 text-sm font-medium text-black-100 dark:text-black">Role</label>
                        <select name="role" id="role"
                            ...
                            <option value="super-admin" {{ ($data->role ?? '') == 'super-admin' ? 'selected' : '' }}>Super
                                Admin</option>
